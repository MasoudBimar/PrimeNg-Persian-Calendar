...
import {NgModule,ElementRef,OnDestroy,Input,Output,SimpleChange,EventEmitter,forwardRef,Renderer2,
...
providers: [DomService,CALENDAR_VALUE_ACCESSOR,DomHandler]
})

export class AppComponent implements OnInit,OnDestroy,ControlValueAccessor  {
  @Input() defaultDate: jalaliMoment.Moment;
  ...
  _locale: LocaleSettings = {
      firstDayOfWeek: 6,
      dayNames: ["شنبه", "دوشنبه", "سه شنبه", "چهارشنبه", "پنج شنبه", "جمعه", "شنبه"],
  ...
  _minDate: jalaliMoment.Moment;
  
  _maxDate:jalaliMoment.Moment;
  ...
  _disabledDates: Array<jalaliMoment.Moment>;
  ...
  @Input() get minDate():jalaliMoment.Moment {
      return this._minDate;
  }
  
  set minDate(date: jalaliMoment.Moment) {
      this._minDate = date;
  ...
  @Input() get maxDate(): jalaliMoment.Moment {
      return this._maxDate;
  }
  
  set maxDate(date: jalaliMoment.Moment) {
      this._maxDate = date;
  ...
  @Input() get disabledDates(): jalaliMoment.Moment[] {
      return this._disabledDates;
  }
  
  set disabledDates(disabledDates: jalaliMoment.Moment[]) {
  ...
  initTime(date: jalaliMoment.Moment) {
  ...
  getFirstDayOfMonthIndex(month: number, year: number) {
      ...
      let day:jalaliMoment.Moment = moment();

      day.set('D',1);
      day.set('M',month);
      day.set('year',year);
      
      let dayIndex = day.jDate() + this.getSundayIndex();
      return dayIndex >= 7 ? dayIndex - 7 : dayIndex;
  }

  getDaysCountInMonth(month: number, year: number) {

      return 32 - this.daylightSavingAdjust(moment(year)).jDate();
  }
  ...
  isDateBetween(start, end, dateMeta) {
      ...
      if (start && end) {
          let date: jalaliMoment.Moment =(dateMeta.year, dateMeta.month, dateMeta.day);
  ...
  parseValueFromString(text: string): jalaliMoment.Moment {
  ...
  parseDateTime(text): jalaliMoment.Moment {
      let date: jalaliMoment.Moment;
      let parts: string[] = text.split(' ');
      
      if (this.timeOnly) {
          date as jalaliMoment.Moment;
  ...
  updateUI() {
      let val:jalaliMoment.Moment = this.value||this.defaultDate || moment();
  ...
  daylightSavingAdjust(date:jalaliMoment.Moment) {
      if (!date) {
          return null;
      }
...
//   exports: [AppComponent,ButtonModule,SharedModule],
